 ...
 ATTACH_TOOLBAR_PROMISE.then(fn => {
  ...
  function handleNetMessage(msg){
    ...
    if (msg.t === "ui" && msg.op === "offer_draw") {
      ...
      const accept = confirm("El rival propone EMPATE. ¿Aceptar tablas?");
      netSend({ t: "ui", op: "draw_response", accepted: !!accept });
    ...
    if (msg.t === "ui" && msg.op === "resign") {
      alert("Tu rival se ha rendido. Has ganado la partida. Comienza una nueva partida.");
      hardRestart();
      return;
    }
  ...
  if (isSpectator) {
    ...
    if ($btnOfferDraw) {
      $btnOfferDraw.addEventListener("click", () => {
        ...
        // 🔇 Se elimina el alert extra "Has propuesto empate..."
      });
    }
    ...
    if ($btnResign) {
      $btnResign.addEventListener("click", () => {
        const ok = confirm("¿Seguro que quieres rendirte y dar la partida por perdida?");
        if (!ok) return;
